...
use zondee_teec::wrapper::{Connection, Operation, ParamNone, ParamTmpRef, Uuid};
...
fn main() -> zondee_teec::Result<()> {
    ...
    let ctx = Default::default();
    let mut _conn = Connection::new(ctx, "HOST", Default::default()).unwrap();
    println!("Connection closses");
    Ok(())
}
